...
for n1=2:length(CRUISE)
    ...
    tablename2 = strcat(rep,'Discrete Underway Chl-a csv/nes-lter-underway-chla-',CRUISE{n1},'.csv');
    ...
    Results=table('Size',[length(Time_discrete) 20],'VariableTypes',...
        ...
        'replicate','chl','phaeo','fluorescence_wetstar_v','fluorescence_ecofl_v',...
        'fluorescence_wetstar','fluorescence_ecofl',...
    ...
    %the fluorescence (exact times) and the fluorescence +/- 5 min the
    %sampling time.
    for n2=1:length(Time_discrete)
        ...
        A1=find(a1==min(a1));
        %Get the position of the +/-5min around the exact matching time
        A1m=[A1-5;A1-4;A1-3;A1-2;A1-1;A1;A1+1;A1+2;A1+3;A1+4;A1+5];
        ...
        %Average/StdDev fluorescence values at +/- 5min
    ...
    %the next cruise
    tablename = strcat(rep,CRUISE{n1},'_chla_matching.csv');
